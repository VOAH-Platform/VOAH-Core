		...
		stage("Build and Push") {
			steps {
				withCredentials([[$class: 'UsernamePasswordMultiBinding',
					...
					sh 'docker buildx build --platform linux/amd64 --tag implude/voah-core-dev:$BUILD_NUMBER --push .'
					sh 'docker buildx build --platform linux/amd64 --tag implude/voah-core-dev:latest --push .'
				}	
			}
		}
    stage("SSH-Deploy") {
					...
					withCredentials([
						...
						sshagent (credentials: ['debian-server']) {
                			sh '''
								...
								cd /mnt/hdd/implude/voah/voah-core
                				docker compose pull
                				docker compose down
                                docker compose up -d
								'
